...
export default function AdminPage() {
  ...
  // Une seule fonction pour tout (re)charger, utilisee aussi
  // apres chaque action : le panneau reste toujours a jour.
  ...
  const winner = useMemo(() => {
    ...
    if (sorted.length > 1 && counts[sorted[0].id] === counts[sorted[1].id])
      return null;
    return sorted[0];
  ...
  const handleReset = async () => {
    if (
      !window.confirm(
        "Table rase : supprimer TOUTES les participations et TOUS les bulletins ?",
      )
    )
      return;
    if (
      !window.confirm("Derniere confirmation. Cette action est irreversible.")
    )
      return;
    try {
    ...
    <div className="min-h-screen bg-slate-950 p-6">
      ...
      <main className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Etat du vote"
            value={election?.isOpen ? "Ouvert" : "Clos"}
          />
        </div>

        {/* Resultats : les barres-photo que tu as imaginees */}
        <section className="space-y-3">
          {winner && (
            <div className="bg-emerald-500/10 border border-emerald-500/40 rounded-2xl p-4 flex items-center gap-3">
              <Trophy className="w-5 h-5 text-emerald-400" />
              <p className="text-emerald-300 text-sm">
                Gagnant actuel : {winner.name} (Liste {winner.listName}) —{" "}
                {counts[winner.id]} voix
              </p>
            </div>
          )}
          {candidates.map((c) => (
